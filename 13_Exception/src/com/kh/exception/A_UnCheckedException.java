...
public class A_UnCheckedException {
	 ...
	 * ArithmeticException : 나누기 연산에서 0으로 나눌 때 발생 - ClassCastException : 허용되지 않는 객체로
	 * 형변환을 시도할 때 발생 - NegativeArraySizeException : 배열을 생성할 때 크기를 음수로 지정하면 발생
	 * 
	 * => RuntimeException 계열 예외는 대부분 예측 가능한 상황에서 발생 그렇기 때문에 if 문 등 조건 처리를 통해 애초에
	 * 예외가 발생하지 않도록 방지 가능!
	 */
	// --- 필드부 --------------
	...
	public void test1() {
		...
		/*
		 * int result = 0; if (n2 !=0) { result = n1 / n2; }
		 * 
		 ...
		 * * try ~ catch
		 * 
		 * try { 예외가 발생될 수 있는 구문 } catch(발생될예외클래스 매개변수) { 해당 예외가 발생했을 때 실행할 구문 }
		 */
		try {
			int result = n1 / n2;
	...
	public void test2() {
		...
		// * 조건 처리
		/*
		 * if(size >= 77) { int[] arr = new int[size];
	...
	public void test3() {
		...
		try {
			...
			//	  모든 자식 예외 발생 시 처리 가능
			System.out.println("인덱스가 잘못되었거나, 배열 크기가 음수인 경우..");
		}
